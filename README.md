...
| Charts | Chart.js 4.4 |
| UI design system | Custom neumorphic design, DM Sans, DM Mono |
...
The platform UI was designed from scratch with a focus on information density, visual calm, and production credibility. The design language draws from Stripe, Linear, Vercel, and Intercom, tools that have established the baseline for what serious software looks like.

The neumorphic design system creates depth without decoration. Cards push out from the background surface using carefully calibrated light and dark shadows, using the same background colour shifted. Interactive elements press inward on activation. Everything uses CSS custom properties for instant light/dark mode switching.

Typography is DM Sans for UI text and DM Mono for data values. Both chosen for their legibility at small sizes and their neutral authority at display scale.

No external UI library. No Tailwind. No component framework. Every element is hand-authored to ensure the design behaves exactly as intended.
